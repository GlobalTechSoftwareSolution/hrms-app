...
class _HrNoticeScreenState extends State<HrNoticeScreen> {
  ...
  String _searchQuery = '';
  String _filter = 'all'; // all, important, with-attachments
  String _userEmail = '';
  ...
  Future<void> _fetchNotices() async {
    ...
    try {
      ...
      // HR role: show notices where notice_to is null (broadcast) or matches HR email
      ...
      debugPrint('Error fetching HR notices: $e');
    } finally {
      if (mounted) {
    ...
    if (_searchQuery.isNotEmpty) {
      ...
      result = result.where((n) {
        return (n['title'] ?? '').toString().toLowerCase().contains(q) ||
            ...
            (n['notice_to'] ?? '').toString().toLowerCase().contains(q);
      }).toList();
    }
  ...
  Widget build(BuildContext context) {
    return DashboardLayout(role: 'hr', child: _buildContent());
  }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Company-wide and HR-specific announcements',
                  style: TextStyle(
                    color: Colors.grey,
        ...
        children: [
          TextField(
            decoration: const InputDecoration(
              hintText:
                  'Search notices by title, message, sender, recipient...',
